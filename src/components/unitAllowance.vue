<template>
<div class="">
    <h4 style="font-weight: bolder" class="pa-5">Unit Allowance</h4>
    <v-card>
        <v-row>
            <v-col cols="12">
                <div class="_header px-10 pt-5">
                    <h4 style="font-size: 18px">
                        Unit Allownace - Approved List
                    </h4>
                    <div>
                        <v-btn style="font-size: 13px; background-color: #17517e; color: #fff" elevation="0" @click="dialog2 = true">Add New2</v-btn>
                    </div>
      ...
      <v-dialog v-model="dialogDelete" max-width="500px">
        <v-card>
            <div class="label">
                <h4><strong style="color:#B11D18;">Warning!</strong>
                    <v-btn text class="pl-4" @click="dialogDelete=false"><img src="../assets/close.png" alt="" /></v-btn>
                </h4>
            ...
            <v-card-title class="text">
                <v-row>
                    <v-col class="col-12 text-center">You are about to delete this,are you sure you</v-col>
                    <v-col class="col-12 text-center"> want to go ahead and delete it.</v-col>
                    <v-col class="col-12 text-center">
                        <v-btn style="background: #B11D18;width:50%"><strong style="color:#fff">ok</strong></v-btn>
                    </v-col>
                    <v-col class="col-12 text-center">
                        <v-btn class="elavation-0" style="background:#fff; width:50%" @click="dialogDelete=false"><strong style="color:#000">Cancel</strong></v-btn>
                    </v-col>
      ...
      <v-dialog v-model="dialogDelete" max-width="500px">
                        <v-card>
                             <div class="label">
                               <h4><strong style="color:#B11D18;">Warning!</strong>  <v-btn text class="pl-4" @click="dialogDelete=false"><img src="../assets/close.png" alt="" /></v-btn></h4>
                            ...
                            <v-card-title class="text">
                                <v-row>
                                    <v-col class="col-12 text-center">You are about to delete this,are you sure you</v-col>
                                    <v-col class="col-12 text-center"> want to go ahead and delete it.</v-col>
                                        <v-col class="col-12 text-center">  <v-btn  style="background: #B11D18;width:50%" ><strong style="color:#fff">ok</strong></v-btn></v-col>
                                            <v-col class="col-12 text-center">  <v-btn class="elavation-0" style="background:#fff; width:50%" @click="dialogDelete=false"><strong style="color:#000">Cancel</strong></v-btn></v-col>
                ...
                <v-form>
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <div class='d-flex flex-column input-holder py-2 my-4'>
                                <span class='input-title'>Allowance Name
                                    <span>*</span>
                                </span>
                                <v-row>
                                    <v-col class="col-10"> <select v-model="allowance_name" placeholder="Allowance Name" style="width:100%">
                                            <option v-for="item in allowanceArr" :key="item.value" v-bind:value="item.value">{{item.text}}</option>
                                        </select></v-col>
                                    <v-col class="col-2"> <img src="../../public/select.svg" alt=""></v-col>
                                </v-row>
                            ...
                            <div class='d-flex flex-column input-holder py-2 my-4'>
                                <span class='input-title'>Value Type
                                    <span>*</span>
                                </span>
                                <v-row>
                                    <v-col class="col-10"> <select v-model="value_type" placeholder="Value Type" style="width:100%">
                                            <option v-for="item in valueType" :key="item.text">
                                                {{item.text}}</option>
                                        </select></v-col>
                                    <v-col class="col-2"> <img src="../../public/select.svg" alt=""></v-col>
                                </v-row>
                            ...
                            <div class='d-flex flex-column input-holder py-3 my-4'>
                                <span class='input-title'>Fixed Amount percentage Basic
                                    <span>*</span>
                                </span>
                                <input v-model="fixed_percentage" type="number" />
                            </div>
                            <div class='d-flex flex-column input-holder py-2 my-4'>
                            ...
                            <div class='d-flex flex-column input-holder py-2 my-4' v-if="showLSE">
                                <span class='input-title'>Life Line
                                    <span>*</span>
                                </span>
                                <v-row>
                                    <v-col class="col-10"><select v-model="life_line" placeholder="Value Type" style="width:100%">
                                            <option v-for="item in lifeLine" :key="item.text">
                                                {{item.text}}</option>
                                        </select></v-col>
                                    <v-col class="col-2"> <img src="../../public/select.svg" alt=""></v-col>
                                </v-row>
                            </div>
                            <v-row class="mb-3"><v-col class="col-md-6 col-sm-12">  <div class='d-flex flex-column input-holder ' v-if="showLimited">
                                <span class='input-title'>Start Date
                                   
                                </span>
                                <input type="date" v-model="start_date">
                            </div></v-col>
                            <v-col class="col-md-6 col-sm-12">   <div class='d-flex flex-column input-holder ' v-if="showLimited">
                                <span class='input-title'>End Date
...
export default {
  ...
  async populateUnitAllowance() {
               ...
               try {
                    const res = await axios({
                        ...
                        data: {
                            ...
                            allowance_code: this.allowance_name,
                            entered_by: "9b2b233117",
                            value_type: this.value_type,
                            application_level_code: this.applicationCode.value,
                            applied_to: category,
                            amount: this.fixed_percentage,
                            life_line: this.life_line,
                 ...
                 console.log("monk");
                   const message = res.data.response.message.message;
                    if (res.data.response.code === "00") {
                        this.showSearchedEmployee = false,
                        console.log("UNIT Allowance", res);
                        const data = res.data.response.message.data;
                        this.items.push({
                            amount_percentage:data.amount,
                            appliication_level: data.application_level,
                            applies_to: data.applies_to,    
                            value_type: data.value_type,
                            life_line: data.allowance_name,
                            start_date: data.regime_name,
                            institu: data.institution,
                            enterby: data.entered_by_name,
                            enteron: currentDate.split("T")[0],
                            status: data.active_status,
                            alowance_name: data.allowance_name,
        ...
        createUnitAllowance() {
          ...
          if (this.life_line = "unlimited"){
              this.start_date = "null", this.end_date = "null"
          }
         if (this.salary_scale_regime == "" || this.allowance_name == "" || this.value_type == "" || this.fixed_percentage == "" ||
                this.applicationCode == "" || this.life_line == "" || this.start_date == "" || this.end_date == "")  {
