<template>
<div class="">
    <h4 style="font-weight: bolder" class="pa-5">Unit Deduction</h4>
    <v-card>
        <v-row>
            <v-col cols="12">
                <div class="_header px-10 pt-5">
                    <h4 style="font-size: 18px">
                        Unit Deduction - Approved List
                    </h4>
                    <div>
...
export default {
  ...
  async populateUnitDeduction() {
            const res = await axios({
                headers: {
                    Authorization: `Bearer ${this.jwt}`,
                },
                method: "get",
                url: `${this.baseURL}/unit_deduction?institution_code=${this.instu_code}`,
            });
             console.log('Result For Unit Deduction', res);
            res.data.response.message.data.forEach((el) => {
              //console.log("List unit Alllowance",el);
               this.items.push({
                            ...
                            unit_deduction_code:el.unit_deduction_code,
            ...
            console.log("MAIN CATEGORY",category);
               try {
                    const res = await axios({
                        ...
                        method: "post",
                        url: `${this.baseURL}/unit_deduction`,
                        data: {
                            institution_code: this.instu_code,
